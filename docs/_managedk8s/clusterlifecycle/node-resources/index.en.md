...
The CPU and memory shown by an OpenStack flavor describe the capacity of the virtual machine. They do not represent the capacity that is available for customer workloads. Every worker node needs capacity for the operating system, Kubernetes itself, and the platform components that run in the cluster.
...
The reservations are applied to the node before customer pods are scheduled. Their values depend on the selected flavor; they are not a fixed percentage of every node.
...
A 4 GiB node therefore does not provide 4 GiB of memory for customer pods. The `s1.small` example reports approximately 2.7 GiB as allocatable before any platform or customer pods are scheduled. On the larger `s1.large` node, approximately 13.1 GiB of the 16 GiB is allocatable before platform and customer pod requests are scheduled.
...
By default, customer clusters run platform components such as Calico, CoreDNS, the OpenStack cloud controller manager, Cinder CSI, `kube-proxy`, Metrics Server, NodeLocal DNS, and backup and defragmentation jobs. Their pod requests count against node allocatable capacity. GPU discovery is optional, and cluster-specific app configuration can change the exact set of components.

The platform bundles declare resource requests for these components and, where appropriate, resource limits. Most platform limits are memory limits; CPU limits are not set for every component. DaemonSet requests apply to each eligible node, while Deployment and job requests apply to the nodes where their pods run. The capacity available for customer pods is therefore the node's allocatable capacity minus the requests of the platform pods scheduled on that node and the requests of other workloads.

## Impact on workloads and autoscaling

Plan worker capacity using node allocatable resources rather than the raw OpenStack flavor size. Define realistic CPU and memory requests for application containers so that the scheduler and the [Cluster Autoscaler](/managedk8s/clusterlifecycle/autoscaling/) can make decisions based on the resources your workloads actually need.
